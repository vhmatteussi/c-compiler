...
bool struct_or_union(Parser *p){
    switch(get_current_token(p)->type){
        case TOK_KW_STRUCT:
        case TOK_KW_UNION:
            get_next_token(p);
            return true;
...
NodesList *struct_declaration_list(Parser *p){
    NodesList *list = NULL;
    Node *element = struct_declaration(p);
    if(!element){
        return NULL;
    }
    while(element){
        push_node_list(p, &list, element);
        element = struct_declaration(p);
    }
    return list;
}

Node *struct_declaration(Parser *p){
    NodesList *spec_qual_list = specifier_qualifier_list(p);
    if(!spec_qual_list){
        return NULL;
    }
    NodesList *struct_dec_list = struct_declarator_list(p);
    if(!struct_dec_list){
        return NULL;
    }
    ...
    return ast_list_duo(p, NODE_STRUCT_DECLARATION, *spec_qual_list, *struct_dec_list);
}
